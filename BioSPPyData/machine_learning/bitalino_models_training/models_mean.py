...
if __name__ == '__main__':
    d = pd.read_csv("../../data/ml_metrics_datasets/*.csv")
    ...
    parameters_lg = {'C': [0.001, 0.01, 0.1, 1, 10, 100, 1000], 'max_iter':[3000]}
    ...
    # Cross validation
    BEST_PARAM_DT, best_estimator_dt = cross_validation.dtree_grid_search(X_train_scaled, y_train, parameters_dt)

    print("Best DT params:", BEST_PARAM_DT)
    BEST_PARAM_LOGREG, best_estimator_logreg = cross_validation.cv(X_train_scaled, y_train, log_reg, parameters_lg,
                                                                   "LOG_REG")
    print("Best LOG REG params:", BEST_PARAM_LOGREG)
    BEST_PARAM_KNN, best_estimator_knn = cross_validation.cv(X_train_scaled, y_train, knn_model, parameters_knn,
                                                             "KNN")
    print("Best KNN params:", BEST_PARAM_KNN)
    BEST_PARAM_SVC, best_estimator_svc = cross_validation.cv(X_train_scaled, y_train, svc_model, parameters_svc,
                                                             "SVC")
    ...
    ypred_dt = dt_model_final.predict(X_test)
    y_predicted.append(ypred_dt)

    ypred_logreg = logreg_model_final.predict(X_test)
    y_predicted.append(ypred_logreg)

    ypred_knn = knn_model_final.predict(X_test)
    y_predicted.append(ypred_knn)

    ypred_svc = svc_model_final.predict(X_test)
