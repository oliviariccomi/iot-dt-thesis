...
if __name__ == '__main__':
    ...
    d.drop(['ID'], axis=1, inplace=True)
    outp.drop(['ID'], axis=1, inplace=True)
    ...
    Xt = outp.drop(
        columns=['STRESS', 'A0', 'A1', 'A2', 'A3', 'A4', 'C0', 'C1', 'C2', 'C3', 'C4', 'HRV_LF', 'HRV_HF', 'HRV_LFHF'])
    ...
    ypred_dt = dt_model_final.predict(Xt_scaled)
    y_predicted.append(ypred_dt)

    ypred_logreg = logreg_model_final.predict(Xt_scaled)
    y_predicted.append(ypred_logreg)

    ypred_knn = knn_model_final.predict(Xt_scaled)
    y_predicted.append(ypred_knn)

    ypred_svc = svc_model_final.predict(Xt_scaled)
    y_predicted.append(ypred_svc)

    ypred_nb = nb_model.predict(Xt_scaled)
    y_predicted.append(ypred_nb)

    #############################
    #       EVALUATION          #
    #############################
    evaluation.evaluate(yt, y_predicted)
    #  Binarize in order to plot ROC curve and Precision vs Recall curve
    le = LabelBinarizer()
    y_test_bin = le.fit_transform(yt)
